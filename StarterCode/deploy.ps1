...
# Varmista ennen deployta, ettei ZIP sisalla Windows-tyylisia polkuja.
$zipCheckStream = [System.IO.File]::OpenRead($zipPath)
$zipCheck = [System.IO.Compression.ZipArchive]::new($zipCheckStream, [System.IO.Compression.ZipArchiveMode]::Read, $false)
try {
    $invalidEntries = $zipCheck.Entries | Where-Object { $_.FullName.Contains('\') }
    if ($invalidEntries.Count -gt 0) {
        $sample = $invalidEntries | Select-Object -First 3 -ExpandProperty FullName
        throw "ZIP sisaltaa virheellisia polkuja (kenoviiva): $($sample -join ', ')"
    }
} finally {
    $zipCheck.Dispose()
    $zipCheckStream.Dispose()
}
